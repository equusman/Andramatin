...
$activity = getMyOpenActivityPhase($params);
...
$_pagination->total = getMyOpenActivityPhaseTotal($params);
?>
<div class="modal-outer container_dlg_activity_list">
<div id="message-bar" ></div>
<div class="filter-block">
	<form id="frm1" class="view-list" method="post" action="../../actions/lookup/phasetitle_list.php">
	<table>
		<tr>
			<th>Filter</th>
	...
	<script>
		...
		table_data = [<?php
			if ($activity!==false) {
				...
				?>
				{checkAll:'<input type="button" data-val="<?php echo $row['phaseid']; ?>" value="Select" onclick="dlg_checkItem(this)"/>', phaseid: "<?php echo $row['phaseid']; ?>", phasename: "<?php echo $row['phasename']; ?>"},
				<?php
				}
			}
		?>
		...
		$(function(){
			table = $("#activitylisttable").tablecontrol({
				...
				colModel: [
					...
					{field: 'phaseid', caption: 'ID', width: '100', sortable: false, cls: 'text-left ', hcls: ""},
					{field: 'phasename', caption: 'Phase Name', width: '', sortable: false, cls: 'text-left ', hcls: ""}
					],
...
<script type="text/javascript">
	function dlg_checkItem(obj) {
		if (typeof dlg_phasetitle_list_callback == 'function') {
			dlg_phasetitle_list_callback({value:$(obj).data('val'), 
